...
CAttackCore::CAttackCore(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObjectUI(pDevice, pContext)
{
}

CAttackCore::CAttackCore(const CAttackCore& rhs)
	: CGameObjectUI(rhs)
{
}

HRESULT CAttackCore::Initialize(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
...
void CAttackCore::Tick(const _float3& _vPosition)
{
...
void CAttackCore::Late_Tick(CInven* pInven, const _double& TimeDelta)
{
	...
	if (PtInRect(&rcUI, ptMouse))
	{
		...
		pInven->Set_On_Mouse(0);
	}

	m_pRendererCom->Add_RenderGroup(CRenderer::RENDER_UI, this);
}

HRESULT CAttackCore::Render()
{
...
void CAttackCore::Upgrade(class CInven* pInven, CAttack* pAttack)
{
	if (3 <= m_inumLevel)
		return;
...
const _uint& CAttackCore::Get_NeedSouls()
{
	switch (m_inumLevel)
	{
	case 0:
...
HRESULT CAttackCore::Add_Components()
{
	...
	if (FAILED(Add_Component(LEVEL_GAMEPLAY, L"Texture_UI_AttackCore0", L"Texture_AttackCore0",
		(CComponent**)&m_pTextures[0], m_pOwner)))
		return E_FAIL;

	if (FAILED(Add_Component(LEVEL_GAMEPLAY, L"Texture_UI_AttackCore1", L"Texture_AttackCore1",
		(CComponent**)&m_pTextures[1], m_pOwner)))
		return E_FAIL;

	if (FAILED(Add_Component(LEVEL_GAMEPLAY, L"Texture_UI_AttackCore2", L"Texture_AttackCore2",
		(CComponent**)&m_pTextures[2], m_pOwner)))
		return E_FAIL;

	if (FAILED(Add_Component(LEVEL_GAMEPLAY, L"Texture_UI_AttackCore3", L"Texture_AttackCore3",
...
HRESULT CAttackCore::Set_ShaderResources()
{
...
CAttackCore* CAttackCore::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CAttackCore* pInstance = new CAttackCore(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created CAttackCore");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CAttackCore* CAttackCore::Clone(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
	CAttackCore* pInstance = new CAttackCore(*this);

	if (FAILED(pInstance->Initialize(iLevelIndex, pOwner, pArg)))
	{
		MSG_BOX("Failed to Cloned CAttackCore");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CAttackCore::Free()
{
