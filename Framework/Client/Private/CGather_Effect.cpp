#include "CGather_Effect.h"

#include "CGameInstance.h"

CGather_Effect::CGather_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject3D(pDevice, pContext)
{
}

CGather_Effect::CGather_Effect(const CGather_Effect& rhs)
	: CGameObject3D(rhs)
{
}

HRESULT CGather_Effect::Initialize(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
...
void CGather_Effect::Tick(const _double& TimeDelta)
{
...
void CGather_Effect::AfterFrustumTick(const _double& TimeDelta)
{
	if (nullptr != m_pRenderer)
		m_pRenderer->Add_RenderGroup(CRenderer::RENDER_NONLIGHT, this);
}

void CGather_Effect::Late_Tick(const _double& TimeDelta)
{
}

HRESULT CGather_Effect::Render()
{
...
void CGather_Effect::Reset_Effects()
{
	for (auto& particle : m_vecParticles)
	{
...
void CGather_Effect::Reset_Particle(STONEPARTICLE& Particle)
{
	Particle.dAge = 0.f;
...
void CGather_Effect::Render_Effect(_fvector vEffectPos)
{
	Reset_Effects();
...
HRESULT CGather_Effect::Add_Components()
{
...
HRESULT CGather_Effect::SetUp_ShaderResources()
{
...
CGather_Effect* CGather_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CGather_Effect* pInstance = new CGather_Effect(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created CGather_Effect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGather_Effect* CGather_Effect::Clone(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
	CGather_Effect* pInstance = new CGather_Effect(*this);

	if (FAILED(pInstance->Initialize(iLevelIndex, pOwner, pArg)))
	{
		MSG_BOX("Failed to Cloned CGather_Effect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CGather_Effect::Free()
{
