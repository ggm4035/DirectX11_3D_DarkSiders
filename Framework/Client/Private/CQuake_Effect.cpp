...
CQuake_Effect::CQuake_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject3D(pDevice, pContext)
{
}

CQuake_Effect::CQuake_Effect(const CQuake_Effect& rhs)
	: CGameObject3D(rhs)
{
}

HRESULT CQuake_Effect::Initialize(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
...
void CQuake_Effect::Tick(const _double& TimeDelta)
{
...
void CQuake_Effect::AfterFrustumTick(const _double& TimeDelta)
{
	if (nullptr != m_pRenderer)
		m_pRenderer->Add_RenderGroup(CRenderer::RENDER_NONLIGHT, this);
}

HRESULT CQuake_Effect::Render()
{
...
void CQuake_Effect::Reset_Effects()
{
	for (auto& particle : m_vecParticles)
	{
...
void CQuake_Effect::Reset_Particle(STONEPARTICLE& Particle)
{
	Particle.dAge = 0.f;
	Particle.dLifeTime = 2.f;
	Particle.isAlive = true;

	Particle.vVelocity = _float4(GetRandomFloat(-3.f, 3.f), GetRandomFloat(5.f, 7.f), GetRandomFloat(-3.f, 3.f), 0.f);

	Particle.vAccel = _float4(0.f, GetRandomFloat(-20.f, -30.f), 0.f, 0.f);

	XMStoreFloat4x4(&Particle.WorldMatrix, XMMatrixIdentity());
}

void CQuake_Effect::Render_Effect(_fvector vEffectPos)
{
	Reset_Effects();
...
HRESULT CQuake_Effect::Add_Components()
{
...
HRESULT CQuake_Effect::SetUp_ShaderResources()
{
...
CQuake_Effect* CQuake_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CQuake_Effect* pInstance = new CQuake_Effect(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created CQuake_Effect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CQuake_Effect* CQuake_Effect::Clone(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
	CQuake_Effect* pInstance = new CQuake_Effect(*this);

	if (FAILED(pInstance->Initialize(iLevelIndex, pOwner, pArg)))
	{
		MSG_BOX("Failed to Cloned CQuake_Effect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CQuake_Effect::Free()
{
