...
CSpark_Effect::CSpark_Effect(const CSpark_Effect& rhs)
	: CGameObject3D(rhs)
{
}

HRESULT CSpark_Effect::Initialize(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
...
void CSpark_Effect::Tick(const _double& TimeDelta)
{
...
void CSpark_Effect::AfterFrustumTick(const _double& TimeDelta)
{
...
void CSpark_Effect::Reset_Effects()
{
	for (auto& particle : m_vecParticles)
	{
...
void CSpark_Effect::Reset_Particle(STONEPARTICLE& Particle)
{
	Particle.dAge = 0.f;
...
void CSpark_Effect::Render_Effect(_fvector vEffectPos)
{
	Reset_Effects();
...
HRESULT CSpark_Effect::Add_Components()
{
...
HRESULT CSpark_Effect::SetUp_ShaderResources()
{
...
CSpark_Effect* CSpark_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	{
		MSG_BOX("Failed to Created CSpark_Effect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CSpark_Effect* CSpark_Effect::Clone(const _uint& iLevelIndex, CComponent* pOwner, void* pArg)
{
	CSpark_Effect* pInstance = new CSpark_Effect(*this);

	if (FAILED(pInstance->Initialize(iLevelIndex, pOwner, pArg)))
	{
		MSG_BOX("Failed to Cloned CSpark_Effect");
		Safe_Release(pInstance);
	}
	return pInstance;
}
