...
class CArmorCore final : public CGameObjectUI
{
private:
	explicit CArmorCore(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CArmorCore(const CArmorCore& rhs);
	virtual ~CArmorCore() = default;
...
public:
	void Upgrade(class CInven* pInven, CDeffence* pDeffence);
...
public:
	static CArmorCore* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	virtual CArmorCore* Clone(const _uint& iLevelIndex, CComponent* pOwner, void* pArg) override;
