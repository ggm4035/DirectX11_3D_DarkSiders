...
HRESULT CGraphic_Device::Ready_Graphic_Device(HWND hWnd, GRAPHICDESC::WINMODE eWinMode, _uint iWinCX, _uint iWinCY, ID3D11Device** ppDeviceOut, ID3D11DeviceContext** ppDeviceContextOut)
{
	...
	ID3D11RenderTargetView*		pRTVs[1] = {
		m_pBackBufferRTV, /*이거 더 추가되나보다*/
	};

	m_pDeviceContext->OMSetRenderTargets(1, pRTVs,
		m_pDepthStencilView); // 랜더타겟은 여러개 들고 있을 수 있지만, 깊이스탠실 버퍼는 하나만 들 수 있다.
	...
	ViewPortDesc.MaxDepth = 1.f;
								// 묶을 뷰포트의 개수, 위에 구조체주소값
	m_pDeviceContext->RSSetViewports(1, &ViewPortDesc);// 즉 뷰포트 여러개 묶어도 됨. (두개를 만들어서 2인용 창 분할 하는 방법이 있다네요[책에서 본거임])
...
HRESULT CGraphic_Device::Ready_SwapChain(HWND hWnd, GRAPHICDESC::WINMODE eWinMode, _uint iWinCX, _uint iWinCY)
{
	...
	SwapChain.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;	// 얘는 렌더타겟용이다.
...
HRESULT CGraphic_Device::Ready_BackBufferRenderTargetView()
{
	...
	/* 내가 앞으로 사용하기위한 용도의 텍스처를 생성하기위한 베이스 데이터를 가지고 있는 객체이다. */
	/* 내가 앞으로 사용하기위한 용도의 텍스처 : ID3D11RenderTargetView, ID3D11ShaderResoureView, ID3D11DepthStencilView */
	ID3D11Texture2D*		pBackBufferTexture = nullptr;

	/* 스왑체인이 들고있던 텍스처를 가져와봐. */
	if (FAILED(m_pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&pBackBufferTexture)))// 아마 여기서 레퍼런스카운트가 증가했을 지도
		return E_FAIL;
