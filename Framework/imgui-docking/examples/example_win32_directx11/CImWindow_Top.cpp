...
void CImWindow_Top::Save(CGameInstance* pGameInstance)
{
    if (ImGui::Button("SAVE", ImVec2(100, 40)))
        Save_Data("Out/File.dat");
}

HRESULT CImWindow_Top::Save_Data(const string& strFilePath)
{
    HANDLE hFile = CreateFileA(strFilePath.c_str(), GENERIC_WRITE, 0, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
